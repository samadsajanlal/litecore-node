...
/**
 * Will return the path and default tescore-node configuration. It will search for the
 * configuration file in the "~/.Teslacoin" directory, and if it doesn't exist, it will create one
...
function getDefaultConfig(options) {
  ...
  var defaultPath = path.resolve(process.env.HOME, './.Teslacoin');
  var defaultConfigFile = path.resolve(defaultPath, './tescore-node.json');
      ...
      servicesConfig: {
        bitcoind: {
          spawn: {
            ...
            exec: path.resolve(__dirname, '../../bin/Teslacoind')
          }
        }
      }
    };
