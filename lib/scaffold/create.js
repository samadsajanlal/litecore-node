...
var BASE_PACKAGE = {
  ...
  readme: 'README.md',
  dependencies: {
    'tescore-lib': '^' + bitcore.version,
    'tescore-node': version
  }
};
...
function createConfigDirectory(options, configDir, isGlobal, done) {
  mkdirp(configDir, function(err) {
    ...
    try {
      fs.writeFileSync(configDir + '/tescore-node.json', configJSON);
      if (!isGlobal) {
 ...
 * @param {String} options.dirname - The name of the tescore node configuration directory
...
function create(options, done) {
  ...
  async.series([
    function(next) {
      // Setup the the tescore-node directory and configuration
